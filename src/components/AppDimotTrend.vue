<template>
  <div
    ...
    :class="{ 'is-fixed': !trendInline, 'is-inline': trendInline }"
...
import DimotTrendsGraph from './DimotTrendsGraph';

const defaultTitle = 'טוען...';

export default {
  name: 'app-trend',
  components: { DimotTrendsGraph },
  props: {
    disabled: {
      type: Boolean,
      default: false,
  ...
  methods: {
    close() {
      ...
      this.$store.commit('changeTrendPopupStatus', { dimotTrend: false });
    },
